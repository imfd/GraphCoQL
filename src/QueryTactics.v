...
Require Import Ssromega.
  ...
  Ltac leq_queries_size :=
    repeat match goal with
           ...
           | [|- context[ (_ < queries_size_aux (_ :: _))%coq_nat] ] =>
             rewrite {2}/queries_size_aux /= -/(queries_size_aux _); simp selection_size
     ...
     rewrite [queries_size_aux [seq _ | _ <- _] ]/queries_size_aux /= -map_comp /funcomp map_id
           ...
           | [|- context [queries_size_aux (merge_pairs_selection_sets ?s (find_valid_pairs_with_response_name ?s ?ts ?rname ?qs)) ]] =>
             let Hfleq := fresh in
             let Hmleq := fresh in
           ...
           | [|- context [ queries_size (merge_selection_sets (find_fields_with_response_name ?rname ?qs)) ]] =>
            ...
            | [|- context [queries_size_aux (merge_pairs_selection_sets ?s (find_pairs_with_response_name ?rname ?qs)) ] ] =>
             let Hfleq := fresh in
             let Hmleq := fresh in
           ...
           | [|- context [queries_size_aux (filter_pairs_with_response_name ?rname ?σs)] ] =>
             let Hfleq := fresh in
